...
| INCREMENT unary_expression {
		$$.loc = currentSymTable->gentemp(($2.type)->middle);
	  	...
	  	if($2.isMatrix){
			...
			quad_array->emit(PLUS, $$.loc->name, $$.loc->name, arg2, $$.loc, 0, $$.loc );
			quad_array->emit($2.loc->name,$$.loc->name, $$.loc, $2.loc);
		}
		else{
...
| DECREMENT unary_expression {
		...
		if($2.isMatrix){
			...
			$$.loc->Type = $$.type;
			$$.loc->size = compute_width($$.type);
			quad_array->emit(MINUS, $$.loc->name, $$.loc->name, arg2, $$.loc, 0, $$.loc );
			quad_array->emit($2.loc->name,$$.loc->name, $$.loc, $2.loc);
		}
	  	else{
	  		char *res = strdup(($2.loc)->name);
...
| additive_expression SUBTRACT multiplicative_expression {
		if(typecheck($1.type, $3.type)){
	  		...
	  		if(($1.type)->middle==3){
	  			$$.loc->Type->left = $1.type->left;
	  			$$.loc->Type->right = newNode(_INT,-1);
	  			$$.loc->Type->right->left = $1.type->right->left;
	  			int m = *($1.type->left);
	  			int n = *($1.type->right->left);
	  			$$.loc->size = m*n*8+8;
	  		}
...
| 	declarator EQUAL initializer {
		if(flag1 == 0){
	  		...
	  		quad_array->emit($1.var, (currentSymTable->table[(currentSymTable->curr_length)-1].name), (currentSymTable->table), 0);
	  	}	
};
